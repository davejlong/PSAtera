...
function Get-AteraAgent {
  ##############
  # If no param is given the function will get the current PC (+15 caracters machinename)
  ##############
  param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$MachineName=(Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters').'NV Hostname'
  )
  if($AgentID){
  ...
  if($MachineName){
    ...
    $localMachineName = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters').'NV Hostname'
    ...
    if ($MachineName -eq $localMachineName -and $machines.Count -gt 1) {
